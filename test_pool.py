...
def main():
    ...
    output_dir = "/home/gzx/data/Dataset/image_caption/coco/x152_grid_pool_6x10"
    ...
    for i, data in enumerate(data_loaders_val):
        ...
        with torch.no_grad():
            feats = model(img)
            feats = F.adaptive_avg_pool2d(feats, (6, 10)).cpu()
            feats = feats.view(2048, -1).transpose(0, 1).numpy()
            # print(feats.shape)
