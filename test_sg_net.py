...
from scene_graph_benchmark.coco_image_dataset import ExtractCocoDataset
import logging
...
def BatchCollator(batch):
    ...
    images = transposed_batch[0][0].unsqueeze(0)
    hs = transposed_batch[1]
    ws = transposed_batch[2]
...
def main():
    ...
    # python -m torch.distributed.launch --nproc_per_node=$NGPUS  test_sg_net.py --config-file sgg_configs/vgattr/vinvl_grid_x152c4.yaml TEST.IMS_PER_BATCH 4
    # python -m torch.distributed.launch --nproc_per_node=2  test_sg_net.py --config-file sgg_configs/vgattr/vinvl_x152c4.yaml TEST.IMS_PER_BATCH 2

    # python test_sg_net.py --config-file sgg_configs/vgattr/vinvl_grid_x152c4.yaml TEST.IMS_PER_BATCH 1
    # python test_sg_net.py --config-file sgg_configs/vgattr/vinvl_grid_x152c4.yaml TEST.IMS_PER_BATCH 1
    ...
    image_dir = "/home/gzx/data/Dataset/image_caption/coco/images"
    output_dir = "/home/gzx/data/Dataset/image_caption/coco/x152_grid_320x500"
    transforms = build_transforms(cfg, is_train=False)

    dataset = ExtractCocoDataset(image_dir, transforms)
    data_loaders_val = make_data_loader(cfg, dataset, is_distributed=distributed)
    start_time = time.time()
    for i, data in enumerate(data_loaders_val):
        img, h, w, img_ids = data
        img_id = img_ids[0]
        ...
        # print(img.shape)
        with torch.no_grad():
            feats = model(img).cpu().view(2048, -1).transpose(0, 1).numpy()
            # feats = model(img).cpu()
            # print(feats.shape)


        np.savez(output_path, feat=feats, hw=hw)
